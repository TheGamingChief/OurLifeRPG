...
if (_type == "export") then {
    ...
    if (_itemart == "Fahrzeug") then {
		...
		["Factory_Log", format ["%1 just got %2 for selling a vehicle!", player getVariable "RealName",_price]] call RM_fnc_LogToServer;
    ...
    if (_itemart == "waffe") then {
        ...
        _price = (_iarr call INV_getitemCostWithTax)*1;
        ['geld',_price] call INV_AddInvItem;
		["Factory_Log", format ["%1 just got %2 for selling a weapon!", player getVariable "RealName",_price]] call RM_fnc_LogToServer;
