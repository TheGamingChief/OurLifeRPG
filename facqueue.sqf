if(typename _this == "ARRAY") exitwith
    ...
    _maxfacworkers = (_arr select 9);
    _buildtimemul  = (_arr select 10);
    ...
    if(count _fqueue > 0)then{call compile format['%1eta = (%1eta - (%1eta/maxfacworkers))*_buildtimemul;', (_fqueue select 0)];};
...
%1eta = %2*_buildtimemul;
if(%1eta > maxmanitime)then{%1eta = maxmanitime};
%1eta = %1eta/(%3workers/5);
if(%1eta > maxmanitime)then{%1eta = maxmanitime};
 
// Increase manufacturing time for items which will otherwise have manufacturing time under minmanitime (default:10)
// by multiplying it by manitimemul (default:5)
//if(%1eta < minmanitime)then{%1eta = %1eta * manitimemul};
_t = %1eta;
