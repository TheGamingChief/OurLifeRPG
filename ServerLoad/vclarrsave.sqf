...
while {true} do {
	...
	for [{_i=0}, {_i < (count INV_PLAYERSTRINGLIST)}, {_i=_i+1}] do {
		INV_PLAYERLIST SET [_i, call compile (INV_PLAYERSTRINGLIST select _i)];
	};
	...
	if (INV_SaveVclArray) then {
		if ((_arrStr2 != format["%1", INV_VehicleArray]) && ((count INV_VehicleArray) > 0)) then {
