...
if (_targetString == player) exitWith
	...
	if (_geld < _transfermoney) then
		...
		["money_to_bank", format ["%1 (%2) has transferred $%3 to their bank at GRID: %4", PlayerName, getPlayerUID player, _transfermoney, _gridPos]] call RM_fnc_LogToServer;
...
if (Kontostand >= _withdraw) then
	...
	["money_withdraw", format ["%1 (%2) has withdrawn $%3 from their bank to money on hand at GRID: %4", PlayerName, getPlayerUID player, _withdraw, _gridPos]] call RM_fnc_LogToServer;
