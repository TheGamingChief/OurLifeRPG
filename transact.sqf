...
if (_targetString == player) exitWith
	...
	if (_geld < _transfermoney) then 
		...
		["money_to_bank", format ["%1 (%2) has transferred $%3 to their bank at GRID: %4", name player, getPlayerUID player, _transfermoney, _gridPos]] call fn_RMLogToServer;
...
if (not((INV_PLAYERSTRINGLIST select _transfer) call ISSE_UnitExists)) exitWith {player groupChat localize "STRS_bank_player_not_ingame";};
...
if (KontoStand < _steuern) then 
	...
	["money_transferred", format ["%1 (%2) has transferred $%3 to %4's (%5) bank at GRID: %6", name player, getPlayerUID player, _transfermoney, name _targetString, getPlayerUID _targetString, _gridPos]] call fn_RMLogToServer;
...
if (Kontostand >= _withdraw) then 
	...
	["money_withdraw", format ["%1 (%2) has withdrawn $%3 from their bank to money on hand at GRID: %4", name player, getPlayerUID player, _withdraw, _gridPos]] call fn_RMLogToServer;
