...
if (_art == "Use") then 
{
	...
	if (IsCop) exitwith {};
	if ((player distance marea2 < 150) or (player distance marea3 < 150) or (player distance marea4 < 150)) then
	{
		
		
		if (_Item == "MarijuanaSeed") then 
			{
			if (!(player distance marea2 < 150)) exitwith {player groupChat "Sorry, This is not the correct place to grow Marijuana!"};
		...
		if (_Item == "HeroinSeed") then 
			{
			if (!(player distance marea3 < 150)) exitwith {player groupChat "Sorry, This is not the correct place to grow Heroin!"};
			[_item, -1] call INV_AddInvItem;
		...
		if (_Item == "CocaineSeed") then 
			{
			if (!(player distance marea4 < 150)) exitwith {player groupChat "Sorry, This is not the correct place to grow Cocaine!"};
			[_item, -1] call INV_AddInvItem;
	...
	{
	player groupchat "You must be atleast 100m away from the center of the drug area.";
	};
