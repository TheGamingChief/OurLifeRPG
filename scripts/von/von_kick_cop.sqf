...
_msg = "NO VOICE ON GLOBAL CHANNEL!"; //message to be displayed to the user
_countdown = true; //show countdown in chat
//CONFIG END

_seconds = 0;
while {true} do {
  waitUntil {(!isNull findDisplay 55) and (!isNull findDisplay 63) and ((ctrlText ((findDisplay 63) displayCtrl 101) == localize "STR_GLOBAL_CHANNEL"))};
  //wait until microphone icon is showing (IDD 55) and channel label is displayed (IDD 63) and channel label says "Global channel" or "Command channel"
