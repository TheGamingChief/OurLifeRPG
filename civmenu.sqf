...
if (Antwort == 1) then
	...
	if ((_geld + kontostand) < _strafe) exitwith
		...
		(format ["server globalChat format [localize ""STRS_civmenu_didnotpayticket"", name %2, %3];", _cop, player, (_strafe call OL_ISSE_str_IntToStr), (_didnotpay call OL_ISSE_str_IntToStr)]) call swag;
	...
	", _cop, player, (_strafe call OL_ISSE_str_IntToStr), (_copticket call OL_ISSE_str_IntToStr)] call swag;
...
if (Antwort == 2) then
	...
	(format ["server globalChat format [localize ""STRS_civmenu_didpaynothing"", name %2];", _cop, player, (_strafe call OL_ISSE_str_IntToStr)]) call swag;
...
if (["INV_InventarArray", "drug"] call INV_StorageHasKindOf) then
	...
	(format ["if (player == %2) then {player groupChat format[localize ""STRS_civmenu_hasdrugs"", %1, %4, (%3/2)];}; titletext [format[localize ""STRS_civmenucheck_haddrugs"", %1, %3], ""plain""];", player, _checkcop, drugsvalue, ceil(drugsvalue/25000)]) call swag;
	[player, "Drug Trafficking"] call OL_player_WarrantAdd;
	player groupChat localize "STRS_civmenucheck_beendrugsearched";

	} else {(format ["if (player == %2) then {player groupChat localize ""STRS_civmenu_hasnodrugs"";};", player, _checkcop]) call swag;};
...
if ((_loopart == "arrest") and (player distance prisonflag <= 70))  then {
	...
	if (!(player call OL_ISSE_IsVictim)) exitwith {(format ["if (player == %1) then {player groupchat localize ""STRS_inventory_checknohands""};", _copobj]) call swag;};
	isstunned = false;
	[] call OL_startup_SetVariables;

	_copPay = [player] call OL_player_WarrantTotal;
	if (_copPay > 0) then {
		format['if (player == %1) then { kontostand = kontostand + %2; player groupChat "This civ had a bounty of $%2! You got that bounty!" }', _copobj, _copPay] call swag;
	};
	...
	(format ["%1 switchmove ""%2"";", player, "normal"]) call swag;
	...
	switch (_exitArt) do {
		case ("JailExit_Escaped"): {
			...
			format ["server globalChat format [localize ""STRS_civmenucheck_breakout"", name %1];", player] call swag;
...
if (_loopart == "inventcheck") then {
	...
	(format ['if (player == %1) then {[0, 0, 0, ["inventorycheck", %2, %3, %4]] execVM "maindialogs.sqf";};',_aktionsStarter, INV_LizenzOwner, INV_InventarArray, player]) call swag;
};

if (_loopart == "patdown") then {
	_aktionsStarter = _this select 1;
	(format ['if (player == %2) then {[] call OL_fnc_PatDown; systemChat format["Your weapons have been removed by %1"];};', name _aktionsStarter, player]) call swag;
};


if (_loopart == "licheck") then {
	_aktionsStarter = _this select 1;
	(format ['if (player == %1) then {[0, 0, 0, ["licensecheck", %2, %3, %4]] execVM "maindialogs.sqf";};',_aktionsStarter, INV_LizenzOwner, INV_InventarArray, player]) call swag;
};
...
{
	(format ['if (player == %1) then {hint "%2 has been stolen from recently";};', _aktionsStarter, player]) call swag;
};
_geld  	 = 'geld' call INV_GetItemAmount;
...
(format ['if (player == %1) then {["geld", %2] call INV_AddInvItem;};hint "%1 stole %2 from %3";',_aktionsStarter, _geld, player]) call swag;
