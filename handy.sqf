...
if (_art == "handy2") then {
	...
	if (not((format["%1", (_smsplayer)]) call OL_ISSE_UnitExists)) exitWith {player groupChat localize "STRS_inv_item_handy_noplayer";};
	if (_geld < INV_smscost)  exitWith {player groupChat localize "STRS_inv_item_handy_keingeld";};
	if ((_smstext call OL_ISSE_str_Length) > 60)     exitWith {player groupChat localize "STRS_inv_item_handy_text_zu_lang";};
	['geld', -(INV_smscost)] call INV_AddInvItem;
