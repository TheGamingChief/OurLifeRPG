...
_etamult    = _facArray select 10;
_workers    = call compile format['%1workers', _queue];
_index = lbAdd [1, localize "STRS_inv_fac_dia_herstellen"];

if(_workers < 1) exitwith {player groupchat "You need must have atleast one worker to use the factory!";};
...
_index = lbAdd [1, format["%1 ($%2, %3 mins)",(_x call INV_getitemName),(floor (_x call INV_getitemBuyCost))*0.5,round(_eta/60)]];
