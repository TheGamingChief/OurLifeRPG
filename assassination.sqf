...
while {true} do
	...
	for [{_i=0}, {_i < (count workplacejob_assassin_serverarray)}, {_i=_i+1}] do
		...
		if (isNull ((workplacejob_assassin_serverarray select _i) select 0)) then
			...
			"if(iscop)then{player sidechat ""The threat to the VIP has been removed""}" call swag;
			sleep 600;
...
format["workplacejob_assassin_serverarray = workplacejob_assassin_serverarray + [[%1, VIPtarget]];", player] call swag;
...
"if (iscop) then {player sideChat ""Someone is trying to kill a government VIP. The target has been marked on the map. Rescue the target before its too late!""};" call swag;
...
while {true} do
	...
	"if(alive player and isciv and player distance assveh <= 150)then{titleText [""The Government is operating in this area! Turn back or you will be shot!"", ""plain down""]};" call swag;
	...
	if (_minutecounter >= 1200) exitWith
		...
		{
		"server globalchat ""Assassination mission not completed within 20mins the mission has ended."";" call swag;
		sleep 3;
		deletevehicle VIPtarget;
		deletemarker "targetmarker";
		deletevehicle assveh;
	...
	if (!alive VIPtarget and alive player) exitWith
		...
		sleep 10;
		"server globalchat ""The VIP target has been killed!"";" call swag;
