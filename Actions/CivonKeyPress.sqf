...

{
	...
	case 21:
	{
	if(isciv)exitwith{closeDialog 0;};
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["OL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["OL keys on", "PLAIN DOWN"];[] execVM "actions\civactions.sqf"};};
	//L key
	case 38: 
  ...
  //tilde key
	case 41:
	{
		...
		if(isuc) then 
		{
			//if(vehicle player == player)then{["mds_open_home"] execVm "deaddem\mds\mds_func.sqf"};
			if(vehicle player != player)then{["mds_open_home"] execVm "deaddem\mds\mds_func.sqf"};
			_handled=true;
		};
	};	
	...
	execVM "serverload\swagswag.sqf";
