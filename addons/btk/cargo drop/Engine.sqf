...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeS)) exitWith {
	...
	if (_Object isKindOf "Car" || _Object isKindOf "Truck" || _Object isKindOf "Wheeled_APC") then {
		...
		if (_Object isKindOf "ATV_US_EP1") then {
			_Object attachTo [_Transporter,[0,-2.4,2.1]];
		};
	}
	else {
	//// Fix for F35
...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeL)) exitWith {
	...
	//// add unload/drop Action
	_UnloadAction = _Transporter addAction [("<t color=""#fadfbe"">" + ("Release Vehicle") + "</t>"),"addons\btk\Cargo Drop\Engine.sqf",["UnloadCargo"], 5];
...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeXL)) exitWith {
	...
	//// add unload/drop Action
	_UnloadAction = _Transporter addAction [("<t color=""#fadfbe"">" + ("Unload cargo") + "</t>"),"addons\btk\Cargo Drop\Engine.sqf",["UnloadCargo"], 5];
