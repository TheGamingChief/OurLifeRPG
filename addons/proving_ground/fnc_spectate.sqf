...
spect = 
{
	...
	format['if(getplayeruid player in Developer_id) then {player sideChat "[Admin Log] Admin %1 has begun Spectating %2"}',name player, _splr] call swag;
	format['if(getplayeruid player in adminlevel4) then {player sideChat "[Admin Log] Admin %1 has begun Spectating %2"}',name player, _splr] call swag;
	[format["[ADMIN_PANNEL_LOG] Admin %1 has begun Spectating %2",name player, _splr]] call fn_LogToServer;
...
{	
	...
	format['if(getplayeruid player in Developer_id) then {player sideChat "[Admin Log] Admin %1 has Stopped Spectating"}',name player] call swag;
	format['if(getplayeruid player in adminlevel4) then {player sideChat "[Admin Log] Admin %1 has Stopped Spectating"}',name player] call swag;
	[format["[ADMIN_PANNEL_LOG] Admin %1 has Stopped Spectating",name player]] call fn_LogToServer;
};