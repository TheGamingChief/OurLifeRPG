...
case 2: {//addweapon
		...
		//format['server globalChat "(ADMIN)%1 Has Spawned A Weapon";', PlayerName] call OL_network_Swag;
		};
	};
...
case 4: {//addMagazine
		...
		if (_uid in AdminTemp) then {
			...
			//format['server globalChat "(ADMIN)%1 Has Spawned A Magazine";', PlayerName] call OL_network_Swag;
		};
	};
