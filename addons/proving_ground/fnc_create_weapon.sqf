...
case 2: {//addweapon
		...
		//format['server globalChat "(ADMIN)%1 Has Spawned A Weapon";', player getVariable "RealName"] call OL_network_Swag;
		};
	};
...
case 4: {//addMagazine
		...
		if (_uid in AdminTemp) then {
			...
			//format['server globalChat "(ADMIN)%1 Has Spawned A Magazine";', player getVariable "RealName"] call OL_network_Swag;
		};
	};
