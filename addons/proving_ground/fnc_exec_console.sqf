...
switch (_mode) do {
	...
	case 3: {//exec command from history
		...
		//format['server globalChat "(ADMIN/DEV)%1 Has Just Executed Command: %2";', player getVariable "RealName", _command] call OL_network_Swag;
		format['diag_log text "ADMIN LOG: %1 Has Executed Command: %2";', player getVariable "RealName", _command];
		["PGSpawn_Log", format ["%1 (%2) has executed command <%3> at %4", player getVariable "RealName", getPlayerUID player, _command, _gridPos]] call RM_fnc_LogToServer;
	};
	case 4: {//exec globally
		...
		processInitCommands;
		format['diag_log text "ADMIN LOG: %1 Has Executed Globally: %2";', player getVariable "RealName", _command];
		["PGSpawn_Log", format ["%1 (%2) has executed command <%3> globally at %4", player getVariable "RealName", getPlayerUID player, _command, _gridPos]] call RM_fnc_LogToServer;
	};
	case 5: {//exec on server
		...
		//format['server globalChat "(ADMIN/DEV)%1 Has Just Executed On Server: %2";', player getVariable "RealName", _command] call OL_network_Swag;
