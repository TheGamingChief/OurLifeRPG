...
spect = 
{
	...
	format['if(getplayeruid player in _Developer_Id) then {player sideChat "[Dev] Dev %1 has begun Spectating %2"}',name player, _splr] call swag;
	[format["[Dev] Dev %1 has begun Spectating %2",name player, _splr]] call fn_LogToServer;
...
{	
	...
	_Developer_Id = ["76561198073512197","76561198122962728"];
	format['if(getplayeruid player in _Developer_Id) then {player sideChat "[Dev] Dev %1 has Stopped Spectating"}',name player] call swag;
	[format["[Dev] Dev %1 has Stopped Spectating",name player]] call fn_LogToServer;
};