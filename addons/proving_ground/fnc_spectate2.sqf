...
spect = 
{
	...
	format['if(getplayeruid player in OL_Developer) then {player sideChat "[Dev] Dev %1 (%2) has begun Spectating %3 (%4)"}', player getVariable "RealName", getPlayerUID player, name _splr, getPlayerUID _splr] call OL_network_Swag;
	["Admin_Log", format ["Dev %1 (%2) has begun Spectating %3 (%4)", player getVariable "RealName", getPlayerUID player, name _splr, getPlayerUID _splr]] call fn_LogToServer;
...
{	
	...
	format['if(getplayeruid player in OL_Developer) then {player sideChat "[Dev] Dev %1 (%2) has stopped Spectating"}', player getVariable "RealName", getPlayerUID player] call OL_network_Swag;
	["Admin_Log", format ["Dev %1 (%2) has stopped Spectating", player getVariable "RealName", getPlayerUID player]] call fn_LogToServer;
	detach player;
