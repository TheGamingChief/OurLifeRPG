...
_vcl 				= _this select 0;
_c 					= 0;
_load 				= _vcl addAction ["Move to rear door position", "addons\heliDoor\mount_vcl.sqf","", 0, false, true];
...
{ 	
		...
		{	
			switch (typeOf _vcl) do {
				case "MH6J_EP1":  {_load = _vcl addAction ["Move to right door", "addons\heliDoor\mount_vcl.sqf", "", 0, false, true];};
			};
			_c = 1;
		};
