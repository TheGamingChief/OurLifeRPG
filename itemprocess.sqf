if (processscriptrunning == 1) exitwith {
  player groupchat "We're Working as fast as we can. Please wait till we are done before giving us another task";
  spamcount = spamcount + 1;
  if (spamcount == 5) then {
    player globalChat "I have spammed the Process script 5 times, ignoring the text that tells me I cannot run this script multiple times"
  };
  if (spamcount == 10) then {
    player globalChat "I have spammed the Process script 10 times, ignoring the text that tells me I cannot run this script multiple times"
  };
};
...
if (!(_license call OL_license_Owns)) exitwith {
  player groupchat "you do not have the license to do this";
