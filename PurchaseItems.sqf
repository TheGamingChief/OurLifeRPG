...
_fn_CreateorAddSomething =
{
	...
	if (_type == "Item") then
	{
		...
		if(_totalCost == 0)exitwith{player groupchat format['You took %1 %2', (_quantity call OL_ISSE_str_IntToStr), _className]};
	};
...
if(_totalCost > _wallet) then
{
	if (_DebitCardCount > 0) then
	{
		...
		else
		{
			...
			["Buy_Log", format ["%1 (%2) has purchased %3 %4 at %5 for $%6 with a debit card.", PlayerName, getPlayerUID player, _quantity, _name, _gridPos, _totalCost]] call RM_fnc_LogToServer;
		};
	}
	else
	{
...
else
{
	...
	["Buy_Log", format ["%1 (%2) has purchased %3 %4 at %5 for $%6", PlayerName, getPlayerUID player, _quantity, _name, _gridPos, _totalCost]] call RM_fnc_LogToServer;
};
