...
if(_totalCost > _wallet) then	
{
	if (_DebitCardCount > 0) then 
	{
		...
		else
		{
			...
			["Buy_Log", format ["%1 (%2) has purchased %3 %4 at %5 for $%6 with a debit card.", name player, getPlayerUID player, _quantity, _name, _gridPos, _totalCost]] call fn_RMLogToServer;
		};
	} 
	else 
	{
...
else 
{
	...
	["Buy_Log", format ["%1 (%2) has purchased %3 %4 at %5 for $%6", name player, getPlayerUID player, _quantity, _name, _gridPos, _totalCost]] call fn_RMLogToServer;
};