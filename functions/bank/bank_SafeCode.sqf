...
if (_this select 0 == "submit") then {
	...
	if (!(call INV_isArmed)) 				exitWith {
		...
		format ['server globalchat "Some idiot just tried to rob the Bank without a weapon!"'] call swag;
 	};
	if (_codeInput != safeCodeSet) 	exitWith {
		...
		sleep 4;
		format['server globalChat "Speedys Security INC Has Reset The Safe Master Code!"'] call swag;
	...
	'titleText [localize "STRS_bank_rob_titlemsg", "plain"]' call swag;
	[] call OL_bank_AlarmSound;

  'if(iscop) then {playsound "beep"; player sideChat "ATTENTION All Officers 10-19 to the Bank, currently being robbed by armed gunmen!";}' call swag;
