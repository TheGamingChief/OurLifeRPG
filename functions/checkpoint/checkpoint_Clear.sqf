...
switch (true) do {
  case ((getPlayerUID player in OL_SwagDevs) || (getPlayerUID player in adminlevel4) || (getPlayerUID player in OL_CheckpointWL)) : {
    format['if((getPlayerUID player in adminlevel4) || (getPlayerUID player in OL_SwagDevs)) then {player groupChat "%1 has cleared a checkpoint (200m)."}', PlayerName] call OL_network_Swag;
    {
  ...
  case (isCop) : {
    if ((_uid in cpl_id) or (_uid in Deputy_id)) then {
        ...
        format['if(isCop) then{player sideChat "%1 has cleared a checkpoint (200m)."}', PlayerName] call OL_network_Swag;
      {
        ...
        ["CheckPointClear_Log", format ["Officer %1 (%2) has Cleared a Checkpoint at %3", PlayerName, getPlayerUID player, _gridPos]] call RM_fnc_LogToServer;

      } else {
        format['if(isCop) then{player sideChat "%1 has attempted to clear a checkpoint (200m). This can only be done by a Corporal or higher."}', PlayerName] call OL_network_Swag;
      };
  };
  case (ismedic) : {
  ...
  case (_uid in Tow_ID) : {
    {
      ...
      ["CheckPointClear_Log", format ["Mechanic %1 (%2) has Cleared a Checkpoint at %3", PlayerName, getPlayerUID player, _gridPos]] call RM_fnc_LogToServer;
  };
};
