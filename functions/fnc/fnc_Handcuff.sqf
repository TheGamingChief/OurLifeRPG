...
if (animationstate civmenuciv == "actspercmstpsnonwrfldnon_interrogate02_forgoten") exitwith {
  ...
  ["HandCuff_Log", format ["%1 (%2) was released by %3 (%4)", civmenuciv getVariable ["RealName", "Error: No Unit"], getPlayerUID civmenuciv, PlayerName, getPlayerUID player]] call RM_fnc_LogToServer;
  format ['
  %1 switchMove "%2";
...
if ("HandCuffs" call INV_GetItemAmount > 0) then {
  ...
  ["HandCuff_Log", format ["%1 (%2) was handcuffed by %3 (%4)", civmenuciv getVariable ["RealName", "Error: No Unit"], getPlayerUID civmenuciv, PlayerName, getPlayerUID player]] call RM_fnc_LogToServer;
  format ['
  %1 switchMove "%2";
  if (player == %1) then {
    isstunned = true;
