...
format['if (iscop) then { player sideChat "The silent alarm at %1 has gone off, go investigate!"; playsound "beep"; };', _stationName] call swag;
...
if (_stationCancel) then {
  switch (_stationWarrant) do {
    case ("Robbed Bar"): {
      ...
      format ['server globalChat "Someone left %1 mid robbery and the owner has called 911 and locked the register!";', _stationName] call swag;
    };
    case ("Robbed Casino"): {
  ...
  format ['server globalChat "Someone just robbed %1!";', _stationName] call swag;
