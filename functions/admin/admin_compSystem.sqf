...
if ((_newMoneyValue >=1) && (_newMoneyValue <=5000000)) then{
  ...
  server globalChat format["(ADMIN)%1 has given %2 $%3", PlayerName, _playerSelected, _newMoneyValue];
  ["Admin_Log", format ["[COMP_SYSTEM] Admin %1 (%2) has given %3 (%4) $%5", PlayerName, getPlayerUID player, _playerSelected getVariable ["RealName", "Error: No Unit"], getPlayerUID _playerSelected, _newMoneyValue]] call RM_fnc_LogToServer;
}else{
